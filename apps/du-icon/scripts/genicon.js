...
async function gen(url) {
  ...
  // sort 避免每次生产的文件不一致
  const files = glob.sync(path.join(__dirname, '../icons/*.svg')).sort((a, b) => a > b)
  ...
  fs.writeFileSync(path.resolve(__dirname, `../src/iconfont-config.json`), JSON.stringify(iconConfig))

  fs.writeFileSync(
    path.resolve(__dirname, '../src/iconfont.css'),
...
gen('https://lf1-cdn-tos.bytegoofy.com/obj/iconpark/svg_7443_183.d74d733c53e94a9f6fc02380e61a0d85.js')
