...
三、二、一 🚀。级联选择器，用于多级联动选择。**数据格式为树形结构**。
...
<template>
  <PreviewBlock title="单组作为弹出组件">
    ...
    <DuButton @click="handleOpenSearch">打开搜索级联选择</DuButton>
    <div>{{ value.join('/') }}</div>
    <DuCascader :options="options" v-model:value="value" v-model:open="open" :show-search="showSearch">
