...
我们内置的一组预设图标，支持线性/填充两种风格。可通过 `name` 直接引用，也可传入自定义 SVG 图标。**图标名称搜索支持模糊匹配**，如 `filled-check` 可简写为 `check`。
...
// 按 display_order 顺序，回退到 Object.keys
const linearIcons = computed(() => {
  const order = iconConfig.display_order?.linear || Object.keys(iconConfig.icons)
  return order.filter(n => n.includes(keyword.value) && !n.includes('filled'))
})

const filledIcons = computed(() => {
  const order = iconConfig.display_order?.filled || Object.keys(iconConfig.icons)
  return order.filter(n => n.includes(keyword.value) && n.includes('filled'))
})
</script>
```
