    ...
    <div v-if="Object.keys(value)?.length > 0" class="result-display">
      <template v-for="(items, key) in value" :key="key">
        <div class="result-group">
          <div class="result-label">{{filterField.find(item=>item.value === key).label}}:</div>
...
import DuDropdown from "../../../packages/dangoui/src/dropdown/Dropdown.vue"
const value = ref({})
...
function getDimensionLabel(key: string) {
  return filterField.find(d => d.value === key)?.label || key
}

function hasGroups(key: string) {
  const dimension = filterField.find(d => d.value === key)
  return dimension && 'groups' in dimension
}

function getGroupLabel(dimensionKey: string, groupKey: string) {
  const dimension = filterField.find(d => d.value === dimensionKey)
...
function handleTagClick(dimensionKey: string, item: any) {
  const dimension = dimensions.find(d => d.value === dimensionKey)
  if (!dimension) return
  if ('options' in dimension) {
    const currentOptions = value.value[dimensionKey] || []
    const index = currentOptions.findIndex((opt: any) => opt.value === item.value)
    if (index > -1) {
      currentOptions.splice(index, 1)
      if (currentOptions.length === 0) {
        delete value.value[dimensionKey]
      }
    }
  } else {
    const groups = value.value[dimensionKey] || {}
