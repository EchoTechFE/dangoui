...
通过 `option-nav` 插槽自定义选择栏的样式和内容。

#snippet
```vue
<template>
  <PreviewBlock title="自定义选择栏">
...
<script setup lang="ts">
import { ref } from 'vue'
import { DuButton, DuTag, DuIcon } from 'dangoui'
import DuDropdown from "../../../packages/dangoui/src/dropdown/Dropdown.vue"
const value = ref({})
const visible = ref(false)
...
.custom-nav {
  display: flex;
  position: relative;
  background: rgba(var(--du-color-primary-rgb), 0.02);
  border-radius: 6px;
}
...
.custom-nav-item.active :deep(.du-icon) {
  opacity: 1;
  color: var(--du-color-primary);
}
...
| 名称         | 类型                    | 说明               |
| ------------ | ----------------------- | ------------------ |
| options      | FilterField[]           | 选项配置数组       |
| currentIndex | number                  | 当前选中的选项索引 |
| onChange     | (index: number) => void | 切换选项的回调函数 |
